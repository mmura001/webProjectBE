...
router.post("/register", Validator("signup"), async (req, res, next) => {
  ...
  db.query(
    "INSERT INTO loginuser (username,email,password,uuid) VALUES (?,?,?,?)",
    [username, email, hash, uuid],
        ...
        if (result.length > 0) {
          bcrypt.compare(password, result[0].password, (error, response) => {
            if (response) {
              ...
              res
                .status(200)
                .json({ username: result[0].username, email: result[0].email });
            } else {
              res.status(403).send({
                error: {
...
router.put("/profileusername", Validator("update"), async (req, res, next) => {
  ...
  db.query(
    "UPDATE loginuser SET username = ? WHERE email = ?;",
    [username, email],
      ...
      if (err) {
        ...
        if (response) {
          ...
          res.status(403).send({
            error: {
              message:
                "Wrong username/password combination!user already exists",
            },
          });
        }
      }
